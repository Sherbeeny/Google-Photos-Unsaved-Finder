...
describe('Userscript Core Logic', () => {
    ...
    it('should add items to an album', async () => {
        mockFetch.mockResolvedValueOnce(createMockApiResponse('E1Cajb', []));
    ...
    it('should handle errors when adding items to an album', async () => {
        mockFetch.mockRejectedValueOnce(new Error('Network error'));
        await expect(userscript.addItemsToAlbum(mockFetch, mockWindowGlobalData, ['photo_1'], 'album_id_2')).rejects.toThrow('Network error');
    });
    ...
    it('should execute the full processing logic', async () => {
        ...
        mockFetch
            ...
            .mockResolvedValueOnce(createMockApiResponse('E1Cajb', [])); // addItemsToAlbum
