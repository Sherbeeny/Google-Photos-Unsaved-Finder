...
### Fixed
- **Critical Initialization Bug:** The script has been radically refactored to ensure the UI always loads instantly, even if the third-party GPTK API is not available. This was achieved by fully decoupling the UI rendering from the asynchronous data loading.
- The `loadAlbums` function now correctly catches `TypeError` if the GPTK API is not present and displays the raw error in the UI log, as per user instruction.
- A race condition with the "Select All" checkbox listener was also resolved.

### Changed
- The `showUI` function now only handles rendering the UI shell and setting a "Loading..." state, before triggering an asynchronous call to `loadAlbums`.
- The E2E test suite was updated to match the new asynchronous flow, testing for the "Loading..." state before asserting the final success or failure states.
