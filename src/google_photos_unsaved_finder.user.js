...
(function() {
    ...
    async function startProcessing(fetch, windowGlobalData, log, getUiState) {
        ...
        if (matchedItems.length > 0) {
          ...
          await addItemsToAlbum(fetch, windowGlobalData, matchedItems, destination);
          log('Done.');
        }
    }
    ...
    function createUI() {
      ...
      if (typeof GM_addStyle === 'function') {
        GM_addStyle(`
          .gpuf-modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; }
          .gpuf-modal { background: white; margin: 5% auto; padding: 20px; border-radius: 8px; max-width: 600px; max-height: 80vh; overflow-y: auto; }
          .gpuf-modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ccc; padding-bottom: 10px; }
          .gpuf-album-list { max-height: 200px; overflow-y: scroll; border: 1px solid #ccc; padding: 10px; margin: 10px 0; }
          .gpuf-log-viewer { margin-top: 10px; padding: 10px; border: 1px solid #ccc; max-height: 150px; overflow-y: scroll; font-family: monospace; font-size: 12px; background: #f0f0f0; }
        `);
      }
